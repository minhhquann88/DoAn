...
export default function ProfilePage() {
    ...
    async function loadFreshData() {
      try {
        ...
        resetProfile({
          ...
          phone: freshData.phoneNumber || '', // Map phoneNumber to phone (if exists)
          bio: freshData.bio || '',
          location: freshData.address || '', // Map address to location (if exists)
        });
  ...
  const onProfileSubmit = async (data: ProfileFormData) => {
    ...
    try {
      ...
      // Step 4: Clear avatar file state and reset input
      setAvatarFile(null);
      setAvatarPreview(null);
      if (fileInputRef.current) {
        fileInputRef.current.value = '';
